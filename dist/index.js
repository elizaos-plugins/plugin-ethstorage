...
// src/environment.ts
var ethstorageEnvSchema = z.object({
...
async function ethstorageConfig(runtime) {
  try {
    const config = {
      ETHSTORAGE_PRIVATE_KEY: runtime.getSetting("ETHSTORAGE_PRIVATE_KEY") || process.env.ETHSTORAGE_PRIVATE_KEY,
      ETHSTORAGE_ADDRESS: runtime.getSetting("ETHSTORAGE_ADDRESS") || process.env.ETHSTORAGE_ADDRESS,
      ETHSTORAGE_RPC_URL: runtime.getSetting("ETHSTORAGE_RPC_URL") || process.env.ETHSTORAGE_RPC_URL
    };
...
var BlobUploader = class _BlobUploader {
  provider;
  wallet;
  kzg;
  static async create(rpc, pk) {
    const uploader = new _BlobUploader(rpc, pk);
    await uploader.init();
    return uploader;
  }
  constructor(rpc, pk) {
...
function stringToHex(s) {
  return ethers2.hexlify(ethers2.toUtf8Bytes(s));
}
async function sendData(RPC, privateKey, address, key, data) {
  ...
  const contract = new ethers2.Contract(address, EthStorageAbi, wallet);
  const hexKey = ethers2.keccak256(stringToHex(key));
  ...
  return (receipt == null ? void 0 : receipt.status) === 1 ? txRes.hash : void 0;
}
var submitDataTemplate = `Respond with a JSON markdown block containing only the extracted values. Use null for any values that cannot be determined.
...
function isTransferContent(content) {
  ...
  return ethers3.isAddress(content.recipient);
}
  ...
  handler: async (runtime, message, state, _options, callback) => {
    ...
    if (content.amount != null && content.recipient != null) {
      try {
        ...
        const provider = new ethers3.JsonRpcProvider(RPC);
        const wallet = new ethers3.Wallet(PRIVATE_KEY, provider);
        const tx = await wallet.sendTransaction({
          to: content.recipient,
          value: ethers3.parseEther(content.amount.toString())
        });
        await tx.wait();
      ...
      elizaLogger2.log("Either amount or recipient not specified");
    }
  },
  examples: [
    [
...
// src/index.ts
var ethstoragePlugin = {
  name: "ethstorage",
  description: "Ethstorage DA publishing plugin",
  providers: [],
  evaluators: [],
  services: [],
  actions: [transfer_default, submitData_default]
};
var index_default = ethstoragePlugin;
export {
  EthStorageAbi,
